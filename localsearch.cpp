...
vector<SearchResult> vecSearchResult;
...
Types::Score LocalSearch::getBestScoreWithParents(const Ordering &ordering, std::vector<int> &parents, std::vector<Types::Score> &scores) const {
  ...
  //cout << ordering << endl;
  for (int i = 0; i < n; i++) {
    const ParentSet &p = bestParent(ordering, pred, i);   // Find best parents
    score += p.getScore();                                // Cumulate the score
    parents[ordering.get(i)] = p.getId();                 // Deliver the parent ID to the parent vector
    scores[ordering.get(i)] = p.getScore();               // Deliver the score to the score vector
    pred[ordering.get(i)] = 1;                            // Mark whether best parents are found

    //cout << ordering.get(i) << " " << parents[ordering.get(i)] << endl;
    //cout << pred << endl;
...
void LocalSearch::genetic(float cutoffTime, int markMAX, int INIT_POPULATION_SIZE, int NUM_CROSSOVERS, int NUM_MUTATIONS, int MUTATION_POWER, int DIV_LOOKAHEAD, int NUM_KEEP, float DIV_TOLERANCE, CrossoverType crossoverType, int greediness, Types::Score opt, ResultRegister &rr) {
  int n = instance.getN();
  ...
  do {
    ...
    numGenerations++;
  } while (mark<markMAX && rr.check() < cutoffTime);
  std::cout << "Generations: " << numGenerations << std::endl;


  for (int i = 0; i < population.getSize(); ++i)
  {
       vecSearchResult.push_back(population.getSpecimen(i));

       std::cout << "score:  " << population.getSpecimen(i).getScore() << std::endl;
...
void LocalSearch::genetic_2(float cutoffTime, int markMAX, int INIT_POPULATION_SIZE, int NUM_CROSSOVERS, int NUM_MUTATIONS, int MUTATION_POWER, int DIV_LOOKAHEAD, int NUM_KEEP, float DIV_TOLERANCE, CrossoverType crossoverType, int greediness, Types::Score opt, ResultRegister &rr) {
  int n = instance.getN();
  SearchResult best(Types::SCORE_MAX, Ordering(n));
  std::deque<Types::Score> fitnesses;
  Population population(*this);
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if (greediness == -1) {
      ...
      std::cout << "score:  " << o.getScore() << std::endl;
...
// Genetic-3-Buckley;
// Genetic-3-Buckley;
SearchResult LocalSearch::genetic_3(float cutoffTime, int INIT_POPULATION_SIZE, int NUM_CROSSOVERS, int NUM_MUTATIONS, int MUTATION_POWER, int DIV_LOOKAHEAD, int NUM_KEEP, float DIV_TOLERANCE, CrossoverType crossoverType, int greediness, Types::Score opt, ResultRegister &rr) {
  int n = instance.getN();
  SearchResult best(Types::SCORE_MAX, Ordering(n));
  std::deque<Types::Score> fitnesses;
  Population population(*this);
...
void LocalSearch::checkSolution(const Ordering &o) {
  ...
  std::vector<int> parents(n);
  std::vector<Types::Score> scores(n);
  getBestScoreWithParents(o, parents, scores);          // Find the best parents and score compatible with the ordering   
 ...
 // cout << mat[1][1] << endl;
    ...
    for (int j = 0; j < parentVars.size(); j++) {
         mat[var][parentVars[j]] = 1;
    }
    for (int j = 0; j < parentVars.size()-1; j++) {
      parentsStr += std::to_string(parentVars[j]) + " ";
      before = before && (inverse[parentVars[j]] < i);

    }

    parentsStr += std::to_string(parentVars[parentVars.size()-1]);
    valid &= before;
    scoreFromParents += p.getScore();
    scoreFromScores += scores[var];
    //std::cout << var << "\t" << parentsStr << std::endl;
...
//fp = fopen("text.txt");
//for (int i=0; i < n; i++){             //  Display the adjacency matrix
//for (int j=0; j<n; j++)
